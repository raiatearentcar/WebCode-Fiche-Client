document.addEventListener('DOMContentLoaded', function() {
    ...
    function validateForm() {
        ...
        // Validation des cases à cocher - vérification plus robuste
        try {
            // Vérifier si la case de traitement des données est cochée
            const dataProcessingCheckbox = document.querySelector('input[name="accept_data_processing"]');
            if (dataProcessingCheckbox && !dataProcessingCheckbox.checked) {
                alert(languageSelect.value === 'fr' ? 
                    'Vous devez accepter le traitement des données personnelles.' : 
                    'You must accept the personal data processing.');
                return false;
            }
        } catch (error) {
            console.error('Erreur lors de la validation des cases à cocher:', error);
            // Continuer malgré l'erreur pour permettre la soumission du formulaire
        }
