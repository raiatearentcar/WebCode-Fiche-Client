    ...
    try {
      ...
      db.all("PRAGMA table_info(clients)", [], (err, tableInfo) => {
        ...
        if (missingColumns.length > 0) {
          ...
          // Exécuter toutes les requêtes ALTER TABLE
          Promise.all(alterTablePromises.map(p => p.catch(e => e)))
            .then(() => {
              // Après avoir ajouté toutes les colonnes, insérer les données
              insertClientData(clientData, pdfPath);
            })
            .catch(error => {
              console.error('Certaines colonnes n\'ont pas pu être ajoutées, mais on continue:', error);
              insertClientData(clientData, pdfPath);
            });
        } else {
